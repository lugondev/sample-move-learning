module LugonSample::LugonToken02 {
    ...
    const TOKEN_NAME: vector<u8> = b"LugonSample";
    const TOKEN_SYMBOL: vector<u8> = b"LUS";
    ...
    struct CapStore has key, store {
        ...
        // anymore.
        mint: Option<MintCapability<LUS>>,
        burn: Option<BurnCapability<LUS>>,
        freeze: Option<FreezeCapability<LUS>>
    }
    ...
    fun init_module(source: &signer) {
        ...
        // We are using our phantom struct to register our shiny LUS with coin module. Internally
        // it will create a CoinInfo struct which holds name, symbol etc for the coin.
        let (cap_burn, cap_freeze, cap_mint) = coin::initialize<LUS>(
            source,
        ...
        // 2. coin::initialize() validates that ONLY WE (meaning the account which defines LUS struct)
        //    can call it.
        ...
        // Anyway, we register our main account for the token - which internally will store CoinStore<LUS>
        // on our account.
        coin::register<LUS>(source);
        ...
        let minted = coin::mint<LUS>(10000000000000000, &cap_mint);
    ...
    public entry fun mint(account: &signer, amount: u64, to: address) acquires CapStore {
        ...
        // This is safe because Coin<LUS> does not have "drop" ability, so
    ...
    fun mint_new_tokens() acquires CapStore {
        ...
        let root = create_account_for_test(@LugonSample);
        ...
        assert!(coin::balance<LUS>(address_of(&user)) == amount, UT_ERR);
    }

    #[test]
    fun init_registers_root_for_coin() {
        // ok, so this one simply tests if init_module() automatically registers
        // our root account for LUS so that we don't have to do it manually later.
        let root = create_account_for_test(@LugonSample);
        init_module(&root);

        assert!(coin::is_account_registered<LUS>(@LugonSample), UT_ERR);
    }
    ...
    fun init_can_be_called_safely_twice() {
        ...
        assert!(exists<CapStore>(@LugonSample), UT_ERR);
    }
}
